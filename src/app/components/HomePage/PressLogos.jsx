    ...
    <section className="bg-white py-10 sm:py-12 dark:bg-zinc-900">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <ul className="grid grid-cols-2 items-center justify-items-center gap-x-10 gap-y-8 sm:grid-cols-4 sm:gap-x-16">
          {logos.map((logo) => (
            <li key={logo.alt} className="w-32 sm:w-40 flex justify-center">
              <img
                src={logo.src}
                alt={logo.alt}
                className="max-h-8 w-auto object-contain opacity-60 grayscale transition hover:opacity-80 hover:grayscale-0 dark:invert dark:opacity-70"
              />
            </li>
          ))}
        </ul>
